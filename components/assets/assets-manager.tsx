...
export function AssetsManager({
  ...
  const [assetToDelete, setAssetToDelete] = useState<AssetWithRelations | null>(
    null,
  );
  const [assetToArchive, setAssetToArchive] =
    useState<AssetWithRelations | null>(null);
  const [assetToUnarchive, setAssetToUnarchive] =
    useState<AssetWithRelations | null>(null);
    ...
    <div className="space-y-3">
      <div>
        ...
        <p className="text-sm text-gray-600">
          Latest portal modules for this organization.
        </p>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Track infrastructure and linked tickets.
          </p>
        </div>
          ...
          {filteredAssets.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {filteredAssets.map(({ asset, ticketCount, lastLinkedAt }) => (
                <Card
                  key={asset.id}
                  className={asset.archived ? "opacity-75" : ""}
                >
                  ...
                  <CardContent className="space-y-3 text-sm text-gray-600">
                    <div className="flex flex-wrap gap-2">
                      ...
                      {asset.site && (
                        <Badge variant="secondary">{asset.site.name}</Badge>
                      )}
                      {asset.area && (
                        <Badge variant="secondary">{asset.area.name}</Badge>
                      )}
                    </div>
                    ...
                    {asset.archived && asset.archivedAt && (
                      <p className="text-xs text-orange-600">
                        ...
                        {asset.archivedByUser &&
                          ` by ${asset.archivedByUser.name || asset.archivedByUser.email}`}
                      </p>
                    )}
                    <div className="text-xs text-gray-500">
                      ...
                      {lastLinkedAt && (
                        <span>
                          {" "}
                          · Last linked {formatDateTime(lastLinkedAt)}
                        </span>
                      )}
                    </div>
                    <div className="flex gap-2">
      ...
      <Dialog
        open={!!assetToArchive}
        onOpenChange={() => setAssetToArchive(null)}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              &quot;? Archived assets will be hidden from the default view but
      ...
      <Dialog
        open={!!assetToUnarchive}
        onOpenChange={() => setAssetToUnarchive(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Unarchive Asset</DialogTitle>
            <DialogDescription>
              Restore &quot;{assetToUnarchive?.name}&quot; to the active assets
              list?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setAssetToUnarchive(null)}
              disabled={isProcessing}
            >
              Cancel
            </Button>
            <Button
              variant="default"
              onClick={() =>
                assetToUnarchive && handleUnarchive(assetToUnarchive)
              }
              disabled={isProcessing}
            >
              {isProcessing ? (
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <br />
              <br />
              Note: Assets with linked tickets cannot be deleted. You must
              unlink all tickets first.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
