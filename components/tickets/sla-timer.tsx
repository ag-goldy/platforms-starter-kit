...
export function SLATimer({
  ...
  const responseDueAt = new Date(
    created.getTime() + slaResponseHours * 3600000,
  );
  const responseRemaining = firstResponseAt
    ? null
    : responseDueAt.getTime() - now.getTime();

  // Resolution SLA
  const resolutionDueAt = new Date(
    created.getTime() + slaResolutionHours * 3600000,
  );
  const resolutionRemaining = resolvedAt
    ? null
    : resolutionDueAt.getTime() - now.getTime();
