...
export async function handleCleanupOrgStorage(data: {
  orgId: string;
}): Promise<JobResult> {
  const { orgId } = data;
  ...
  try {
    ...
    for (const path of paths) {
      try {
        ...
        console.error(
          `[CleanupOrgStorage] Failed to cleanup path ${path}:`,
          error,
        );
        failedCount++;
      }
    ...
    console.log(
      `[CleanupOrgStorage] Completed for org ${orgId}. Paths processed: ${deletedCount}, failed: ${failedCount}`,
    );
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Unknown error during storage cleanup",
    };
  }
}
