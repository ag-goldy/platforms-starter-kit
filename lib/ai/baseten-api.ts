...
export function filterTechnologyContent(content: string): {
  isValid: boolean;
  reason?: string;
} {
  ...
  for (const pattern of blockedPatterns) {
    if (pattern.test(lowerContent)) {
      return {
        isValid: false,
        reason:
          "Content must be technology-related (networking, servers, software, IT support, etc.)",
      };
    }
  }
...
export async function parseTechResource(
  url: string,
  ...
  try {
    ...
    if (!isAllowed) {
      return {
        error:
          "Web parsing limited to technology documentation and resources only.",
      };
    }
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a technical writer creating knowledge base articles for IT support. Focus on clear, actionable solutions for technology issues.",
          },
...
Provide reasoning and confidence level (0-1).`;
    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an IT support analyst. Classify tickets accurately based on urgency and type.",
          },
          { role: "user", content: prompt },
        ],
        max_tokens: 500,
      ...
      const categoryMatch = analysis.match(
        /Category:\s*(INCIDENT|SERVICE_REQUEST|CHANGE_REQUEST)/i,
      );
      ...
      return {
        ...
        category:
          (categoryMatch?.[1] as
            | "INCIDENT"
            | "SERVICE_REQUEST"
            | "CHANGE_REQUEST") || "SERVICE_REQUEST",
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a professional IT support agent. Provide helpful, technical responses.",
          },
          { role: "user", content: prompt },
        ],
        max_tokens: 800,
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a KB search engine. Return relevant technical articles.",
          },
          { role: "user", content: prompt },
        ],
        max_tokens: 600,
...
export function withTechFilter<
  T extends (...args: string[]) => Promise<unknown>,
>(fn: T): T {
  return (async (...args: Parameters<T>) => {
    try {
      ...
      if (
        ...
        error.message.includes("technology-related")
      ) {
        throw error;
      }
...
export async function generateKBAnswer(
  query: string,
  context: Array<{ title: string; excerpt: string; url?: string }>,
): Promise<string> {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a helpful technical assistant. Use provided knowledge base context when available.",
        },
        { role: "user", content: prompt },
      ],
      temperature: 0.2,
