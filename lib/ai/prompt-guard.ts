...
export function detectPromptInjection(userInput: string): PromptGuardResult {
  ...
  const injectionPatterns = [
    ...
    {
      pattern: /disregard\s+(all\s+)?(previous|above|prior)/gi,
      threat: "instruction_override",
      severity: "high",
    },
    {
    ...
    {
      pattern: /stop\s+(following|obeying)\s+(instructions?|rules?)/gi,
      threat: "instruction_override",
      severity: "high",
    },
    ...
    {
      pattern: /act\s+as\s+(?:a|an|the)\s+/gi,
      threat: "role_manipulation",
      severity: "high",
    },
    {
      pattern: /pretend\s+(?:to\s+be|you'?re)\s+/gi,
      threat: "role_manipulation",
      severity: "medium",
    },
    {
      pattern: /switch\s+to\s+(?:admin|developer|system|root)/gi,
      threat: "privilege_escalation",
      severity: "high",
    },
    {
      pattern: /become\s+(?:a|an)\s+/gi,
      threat: "role_manipulation",
      severity: "medium",
    },
    ...
    {
      pattern:
        /(?:show|reveal|display|print|output|repeat)\s+(?:your|the|system)\s+(?:prompt|instructions?|rules?|system\s+message)/gi,
    ...
    {
      pattern: /tell\s+me\s+your\s+(?:system|initial)\s+/gi,
      threat: "prompt_extraction",
      severity: "medium",
    },
    {
      pattern:
        /repeat\s+(?:everything|all)\s+(?:you|you've)\s+(?:said|written)/gi,
      threat: "prompt_extraction",
      severity: "medium",
    },
    ...
    {
      pattern:
        /(?:list|show|give|tell)\s+(?:me\s+)?(?:all|every)\s+(?:users?|customers?|organizations?|tenants?|tickets?|emails?)/gi,
      threat: "data_exfiltration",
      severity: "high",
    },
    {
      pattern:
        /(?:dump|export|extract)\s+(?:the\s+)?(?:database|data|records?|table)/gi,
      threat: "data_exfiltration",
      severity: "high",
    },
    {
      pattern: /(?:show|list)\s+all\s+(?:entries?|rows?|records?)/gi,
      threat: "data_exfiltration",
      severity: "high",
    },
    {
    ...
    // Encoding tricks
    {
      pattern: /(?:base64|hex|rot13|binary)\s*(?:decode|encode)/gi,
      threat: "encoding_attack",
      severity: "medium",
    },
    {
      pattern: /decode\s+this[:\s]/gi,
      threat: "encoding_attack",
      severity: "medium",
    },
    ...
    {
      pattern: /```(?:system|assistant|admin|user)/gi,
      threat: "delimiter_injection",
      severity: "high",
    },
    {
      pattern: /<\/?(?:system|admin|internal|prompt)>/gi,
      threat: "tag_injection",
      severity: "high",
    },
    {
      pattern: /\[SYSTEM\]|\[ADMIN\]|\[PROMPT\]/gi,
      threat: "delimiter_injection",
      severity: "medium",
    },
    ...
    {
      pattern:
        /(?:other|different|another)\s+(?:organization|tenant|company|customer)/gi,
      threat: "cross_tenant_probe",
      severity: "medium",
    },
    {
      pattern:
        /(?:switch|change|access)\s+(?:to\s+)?(?:organization|tenant|company)/gi,
      threat: "cross_tenant_probe",
      severity: "high",
    },
    {
      pattern:
        /(?:show|list)\s+(?:other|all)\s+(?:orgs?|organizations?|tenants?)/gi,
      threat: "cross_tenant_probe",
      severity: "high",
    },
    ...
    {
      pattern: /DAN\s+mode|jailbreak|"developer"\s+mode/gi,
      threat: "jailbreak_attempt",
      severity: "high",
    },
    {
      pattern: /bypass\s+(?:your|the)\s+(?:filter|rules|restrictions)/gi,
      threat: "jailbreak_attempt",
      severity: "high",
    },
    {
      pattern: /override\s+(?:your|the|system)/gi,
      threat: "jailbreak_attempt",
      severity: "high",
    },
    {
...
export function getSafeResponse(threats: string[]): string {
  ...
  if (
    threats.includes("instruction_override") ||
    threats.includes("jailbreak_attempt")
  ) {
