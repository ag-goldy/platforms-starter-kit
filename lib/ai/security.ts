...
export function validateDataAccess(
  ...
  if (access === "none") {
    return {
      allowed: false,
      scope: "none",
      reason: `${dataType} access denied for ${context.interface} interface`,
    };
  }
  ...
  if (context.interface === "customer") {
    if (!context.orgId) {
      return {
        allowed: false,
        scope: "none",
        reason: "No org context for customer request",
      };
    }
...
export function sanitizeResponse(
  ...
  if (context.interface === "customer") {
    piiPatterns.push({
      pattern: /\b(?:agent|admin|internal)\s+\S+@\S+\b/gi,
      type: "internal_email",
      replacement: "[staff]",
    });
  }
...
export function anonymizeInternalUser(content: string): string {
  ...
  return content
    .replace(
      /\b(agent|admin|staff|support engineer)\s+\S+@\S+/gi,
      "support team",
    )
    .replace(
      /\b(agent|admin|staff|support engineer)\s+[A-Z][a-z]+/gi,
      "support team",
    )
...
export async function sanitizeResponseWithOrgRules(
  ...
  const {
    sanitized: staticSanitized,
    piiDetected: staticDetected,
    piiTypes: staticTypes,
...
export async function buildSecurityContext(
  ...
  if (interfaceType === "customer" && !orgId) {
    ...
    const subdomainMatch = host.match(
      /^([a-z0-9-]+)\.atlas\.agrnetworks\.com$/i,
    );
    if (subdomainMatch) {
