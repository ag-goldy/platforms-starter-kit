 ...
 */
export async function handleAIDataDeletionRequest(
  userId: string,
  orgId?: string,
): Promise<{
  success: boolean;
  ...
  await db.delete(orgAIMemory).where(eq(orgAIMemory.addedBy, userId));
...
export async function handleOrgAIDataDeletion(orgId: string): Promise<{
  ...
  await db.delete(orgAIMemory).where(eq(orgAIMemory.orgId, orgId));
