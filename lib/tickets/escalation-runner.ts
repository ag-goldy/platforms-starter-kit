...
export async function executeEscalationActions(ticketId: string, orgId: string, actionsJson: Record<string, any>) {
  ...
  if (actionsJson.raise_priority) {
    ...
    await db.insert(ticketEvents).values({
      ...
      payloadJson: { priority: actionsJson.raise_priority, reason: 'escalation' }
    });
  }
