...
async function getAvailableAgents(orgId: string): Promise<{ id: string; role: string }[]> {
...
async function getAgentWorkloads(orgId: string, agentIds: string[]): Promise<Map<string, number>> {
  ...
  const workloads = await db
    ...
    .where(
      and(
        ...
        sql`${tickets.assigneeId} IN (${sql.join(agentIds.map(id => sql`${id}`), sql`, `)})`,
...
export async function autoAssignTicket(
  ...
  if (agents.length === 0) {
    return { assigneeId: null, reason: 'No available agents', method: 'none', confidence: 0 };
  }

  if (agents.length === 1) {
    return { assigneeId: agents[0].id, reason: 'Only available agent', method: 'fallback', confidence: 100 };
  }
