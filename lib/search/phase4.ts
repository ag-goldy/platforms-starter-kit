...
export async function searchKnowledgeAndAssets(
  ...
  return withOrgScope(orgId, async (scopedOrgId) => {
    ...
    const articleResults: Phase4SearchResult[] = articleRows.map((article) => ({
      ...
      href: subdomain
        ? `/s/${subdomain}/kb/${article.slug}`
        : `/kb-public/${scopedOrgId}/${article.slug}`,
    ...
    const assetResults: Phase4SearchResult[] = assetRows.map((asset) => ({
      ...
      title: asset.name,
      excerpt:
        [asset.hostname, asset.ipAddress, asset.serialNumber, asset.model]
          .filter(Boolean)
          .join(" | ") || null,
      href: subdomain
        ? `/s/${subdomain}?view=assets&asset=${asset.id}`
        : `/app/organizations/${scopedOrgId}/assets/${asset.id}`,
