...
import { setPresence, removePresence, getPresence, subscribeToPresence, type UserPresence } from '@/lib/redis/presence';
...
export function useTicketPresence({ ticketId, userId, enabled = true }: UseTicketPresenceOptions) {
  ...
  const checkForCollision = useCallback(async (currentCommentCount: number): Promise<{ hasCollision: boolean; newComments: number }> => {
    if (!enabled) return { hasCollision: false, newComments: 0 };

    // Get current presence
    const presences = await getPresence(ticketId);
    const otherEditors = presences.filter(p => p.userId !== userId && p.action === 'editing');
  ...
  return {
    ...
    hasOtherEditors: viewers.some(v => v.action === 'editing' || v.action === 'typing'),
