export type TenantRole = 'owner' | 'admin' | 'agent_lead' | 'agent' | 'analyst' | 'end_user' | 'system';
...
// Resource Types
export type TicketResource = { type: 'ticket'; orgId: string; assigneeId?: string; teamId?: string; requesterId: string };
export type KbArticleResource = { type: 'kb_article'; orgId: string; visibility: string; restrictedTeamIds?: string[] };
export type AssetResource = { type: 'asset'; orgId: string };
export type OrgResource = { type: 'org'; orgId: string };

export type Resource = TicketResource | KbArticleResource | AssetResource | OrgResource;
...
export function requirePermission(action: Action, resource: Resource, ctx: AuthCtx): asserts ctx is AuthCtx & { allowed: true } {
  ...
  if (ctx.kind === 'impersonation' && ctx.impersonatingOrgId === resource.orgId) {
  ...
  if (resource.type === 'ticket') {
    if (action === 'ticket.reply' || action === 'ticket.edit') {
      ...
      throw new PermissionError('Insufficient permissions to modify this ticket');
    }
  }
...
type LegacyRole = 'ADMIN' | 'AGENT' | 'READONLY' | 'CUSTOMER_ADMIN' | 'REQUESTER' | 'VIEWER';

export function checkInternalRole(userRole: string, allowedRoles?: LegacyRole[]): void {
...
export async function requireInternalRole(allowedRoles?: LegacyRole[]) {
  ...
  const internalRole = ((context.user as { role?: string }).role || 'AGENT') as LegacyRole;
...
export async function requireOrgRole(orgId: string, allowedRoles?: LegacyRole[]) {
  const { getRequestContext } = await import('@/lib/auth/context');
  ...
  const membership = context.membership?.orgId === orgId
    ? context.membership
  ...
  if (allowedRoles?.length && !allowedRoles.includes(membership.role as LegacyRole)) {
...
export async function requireOrgMemberRole(orgId?: string, allowedRoles?: LegacyRole[]) {
  if (orgId) {
...
export async function canManageOrgSettings(userId: string, orgId: string | null | undefined): Promise<boolean> {
...
export async function canManageTickets(userId: string, orgId: string | null | undefined): Promise<boolean> {
  if (!orgId) return false;
  try {
