...
function ipMatchesRule(ip: string, rule: string) {
  ...
  const toInt = (value: string) => value
    .split('.')
...
export async function getImpersonationState(): Promise<ImpersonationState | null> {
  ...
  try {
    const state = JSON.parse(Buffer.from(value, 'base64url').toString('utf8')) as ImpersonationState;
...
export async function setImpersonationState(state: ImpersonationState, durationMinutes = 30) {
  const cookieStore = await cookies();
  cookieStore.set(IMPERSONATION_COOKIE, Buffer.from(JSON.stringify(state), 'utf8').toString('base64url'), {
    httpOnly: true,
    sameSite: 'lax',
...
export async function logPlatformAudit(input: {
  platformAdminId: string;
  orgId?: string | null;
  action: 'ORG_CREATED' | 'ORG_UPDATED' | 'ORG_DISABLED' | 'ORG_ENABLED' | 'ORG_DELETED';
