...
const optional = [
  ...
  { key: 'MICROSOFT_GRAPH_CLIENT_SECRET', warning: 'Email integration disabled' },
  { key: 'MICROSOFT_GRAPH_TENANT_ID', warning: 'Email integration disabled' },
  { key: 'GRAPH_WEBHOOK_SECRET', warning: 'Graph webhook clientState not secured' },
...
export function validateEnv() {
  ...
  for (const { key, warning } of optional) {
    if (!process.env[key]) {
      ...
      if (key === 'INBOUND_EMAIL_SECRET' && process.env.MICROSOFT_GRAPH_TENANT_ID) {
  ...
  const hasRedis = process.env.REDIS_URL || process.env.TCKREDIS_REDIS_URL || process.env.KV_REST_API_URL;
  if (!hasRedis) {
    console.warn('⚠️  No Redis configuration found — Caching will use in-memory fallback');
