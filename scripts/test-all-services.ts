...
async function testEmailConfiguration() {
  ...
  if (graphConfigured) {
    ...
    log(
      "EMAIL",
      `   Tenant ID: ${process.env.MICROSOFT_GRAPH_TENANT_ID?.substring(0, 8)}...`,
      "info",
    );
    log(
      "EMAIL",
      `   From: ${process.env.EMAIL_FROM_ADDRESS || "help@agrnetworks.com"}`,
      "info",
    );
    ...
    log(
      "EMAIL",
      "⚠️ No email service configured - using console fallback",
      "warn",
    );
...
async function testUsers() {
  ...
  try {
    ...
    log(
      "USERS",
      `✅ Found ${internalUsers.length} internal user(s):`,
      "success",
    );
...
async function testAuth() {
  ...
  log(
    "AUTH",
    `   Trust Host: ${process.env.AUTH_TRUST_HOST || "not set"}`,
    "info",
  );
...
async function main() {
  ...
  const results: Record<
    string,
    boolean | { graphConfigured: boolean; smtpConfigured: boolean }
  > = {
  ...
  // Email sending test
  const emailConfig = results.email as {
  ...
  if (emailConfig.graphConfigured || emailConfig.smtpConfigured) {
    ...
    log(
      "EMAIL",
      "Skipping email send test (no email service configured)",
      "warn",
    );
