...
const GLOBAL_ADMIN_EMAIL =
  process.env.GLOBAL_ADMIN_EMAIL || "ag@agrnetworks.com";
const GLOBAL_ADMIN_PASSWORD = process.env.GLOBAL_ADMIN_PASSWORD;
const GLOBAL_ADMIN_NAME =
  process.env.GLOBAL_ADMIN_NAME || "AGR Global Administrator";

if (!GLOBAL_ADMIN_PASSWORD) {
  console.error(
    "Error: GLOBAL_ADMIN_PASSWORD environment variable is required",
  );
  process.exit(1);
}
...
async function createGlobalAdmin() {
  ...
  if (superAdminGroup) {
    const existingMembership =
      await db.query.internalGroupMemberships.findFirst({
        where: and(
