...
import { signWebhookPayload, generateWebhookSecret } from '@/lib/webhooks/queries';
import { getRequestContext, type RequestContext } from '@/lib/auth/context';
import { requireInternalRole, AuthorizationError } from '@/lib/auth/permissions';
...
describe('webhook payload signing', () => {
  it('produces a deterministic HMAC-SHA256 signature', () => {
    ...
    const payload = JSON.stringify({ event: 'ticket.created', data: { id: '1' } });
