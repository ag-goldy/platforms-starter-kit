...
function Status({ ok }: { ok: boolean }) {
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-xs font-medium ${ok ? "bg-emerald-400 text-zinc-950" : "bg-rose-500 text-white"}`}
    >
      {ok ? "OK" : "FAIL"}
    </span>
  );
...
export default async function PlatformHealthPage() {
  ...
  if (isRedisConfigured) {
    try {
      ...
      redisStatus = {
        ok: (await redis.get(key)) === "1",
        message: "Connected",
      };
    } catch (error) {
      redisStatus = {
        ok: false,
        message: error instanceof Error ? error.message : "Redis failed",
      };
    }
  }
  ...
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    try {
      ...
      blobStatus = {
        ok: false,
        message: error instanceof Error ? error.message : "Blob failed",
      };
    }
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-400">
          Platform-level service checks and operational counters.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="border-zinc-800 bg-zinc-900 text-zinc-100">
          <CardHeader>
            <CardTitle className="text-sm text-zinc-400">Tenants</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-semibold">
            {Number(tenantCount[0]?.count || 0)}
          </CardContent>
        </Card>
        <Card className="border-zinc-800 bg-zinc-900 text-zinc-100">
          <CardHeader>
        ...
        <Card className="border-zinc-800 bg-zinc-900 text-zinc-100">
          <CardHeader>
            <CardTitle className="text-sm text-zinc-400">Failed Jobs</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-semibold">
            {Number(failedJobCount[0]?.count || 0)}
          </CardContent>
        </Card>
      </div>
    </div>
