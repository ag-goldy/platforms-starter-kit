...
import {
  discardAdminFailedJobAction,
  retryAdminFailedJobAction,
} from "../actions";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-400">
          Retry or discard dead-letter jobs.
        </p>
      </div>
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs text-zinc-500">
                      {job.jobId} · attempts {job.attempts}/{job.maxAttempts} ·
                      failed {job.failedAt.toLocaleString()}
                    </div>
                    <pre className="mt-3 max-h-28 overflow-auto rounded bg-rose-950/50 p-3 text-xs text-rose-100">
                      {job.error}
                    </pre>
                    <details className="mt-2 text-xs text-zinc-500">
                      <summary className="cursor-pointer">Payload</summary>
                      <pre className="mt-2 max-h-48 overflow-auto rounded bg-zinc-900 p-3">
