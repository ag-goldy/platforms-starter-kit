...
export default async function PlatformAuditPage({
  ...
  if (params.action)
    filters.push(
      eq(
        auditLogs.action,
        params.action as (typeof auditLogs.action.enumValues)[number],
      ),
    );
  if (params.dateFrom)
    filters.push(gte(auditLogs.createdAt, new Date(params.dateFrom)));
  if (params.dateTo)
    filters.push(lte(auditLogs.createdAt, new Date(params.dateTo)));
  if (params.actor) {
    filters.push(
      or(
  ...
  const [tenants, logs] = await Promise.all([
    ...
    db
      ...
      .leftJoin(
        platformAdmins,
        eq(platformAdmins.id, auditLogs.platformAdminId),
      )
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-400">
          Search platform and tenant activity across all organizations.
        </p>
      </div>
              ...
              {logs.map((log) => (
                <tr key={log.id}>
                  ...
                  <td className="text-zinc-400">
                    {log.platformAdminEmail || log.userEmail || "system"}
                  </td>
                  <td>{log.action}</td>
                  <td className="max-w-xl truncate text-zinc-500">
                    {log.details}
                  </td>
                </tr>
              ))}
