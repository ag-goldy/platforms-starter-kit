...
const createIncidentSchema = z.object({
  ...
  componentStatuses: z.record(z.enum(['operational', 'degraded_performance', 'partial_outage', 'major_outage'])).optional(),
});
...
export async function GET(request: NextRequest) {
  try {
    ...
    const incidents = await client.listIncidents({ status: status || undefined, limit });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const canManage = await canManageTickets(session.user.id, session.user.orgId);
    if (!canManage) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
    }
    ...
    const { name, status, impact, body: incidentBody, componentIds, componentStatuses } = validated.data;
