...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!expectedClientState) {
      console.error('[SECURITY] GRAPH_WEBHOOK_SECRET is not configured - rejecting Graph notification');
      return NextResponse.json({ error: 'Graph webhook not configured' }, { status: 503 });
    }
    ...
    for (const notification of body.value) {
      ...
      if (!constantTimeEquals(String(notification.clientState || ''), expectedClientState)) {
        console.error('[Graph Webhook] Invalid clientState - possible spoofing attempt');
        continue;
      }
      ...
      if (notification.changeType === 'created' && notification.resourceData) {
        ...
        if (email) {
          ...
          if (result.success) {
            console.log('[Graph Webhook] Processed email:', result.ticketKey, 'isReply:', result.isReply);
          } else {
            console.error('[Graph Webhook] Failed to process email:', result.error);
          }
        } else {
