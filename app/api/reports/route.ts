 ...
 */
async function getTicketVolumeReport(orgId: string, startDate: Date, endDate: Date) {
 ...
 */
async function getAgentPerformanceReport(orgId: string, startDate: Date, endDate: Date) {
 ...
 */
async function getSLAComplianceReport(orgId: string, startDate: Date, endDate: Date) {
  const results = await db
    .select({
  ...
  return NextResponse.json({
    data: results.map(r => ({
      ...
      responseCompliance: r.total > 0 ? Math.round((Number(r.metResponse) / Number(r.total)) * 100) : 100,
      resolutionCompliance: r.total > 0 ? Math.round((Number(r.metResolution) / Number(r.total)) * 100) : 100,
    }))
  });
}
 ...
 */
async function getCategoryDistributionReport(orgId: string, startDate: Date, endDate: Date) {
  const results = await db
    .select({
  ...
  return NextResponse.json({ data: results.map(r => ({ category: r.category, count: Number(r.count) })) });
}

/**
 * Response Time Report - average first response time
 */
async function getResponseTimeReport(orgId: string, startDate: Date, endDate: Date) {
  const results = await db
    .select({
      date: sql<string>`DATE(${tickets.createdAt})`,
 ...
 */
async function getResolutionTimeReport(orgId: string, startDate: Date, endDate: Date) {
  const results = await db
    .select({
      date: sql<string>`DATE(${tickets.createdAt})`,
 ...
 */
async function getTopRequestersReport(orgId: string, startDate: Date, endDate: Date) {
  const results = await db
    .select({
