  ...
  try {
    ...
    if (!subscriptionId) {
      ...
      const emailSubscription = subscriptions.find(
        s => s.resource.includes('/messages') && s.notificationUrl.includes('/api/graph/notifications')
      );
    ...
    if (hoursUntilExpiry < 24) {
      console.log('[Cron Graph] Subscription expires in', hoursUntilExpiry.toFixed(1), 'hours, renewing...');
