 ...
 * Keyed by org SUBDOMAIN (not internal ID) to avoid exposing internal identifiers.
...
export async function GET(
  ...
  try {
    ...
    // Validate org exists and is active by subdomain (not internal ID).
    // Using subdomain prevents internal UUID enumeration.
    ...
    // Fetch status data (with caching keyed by subdomain)
    const data = await fetchStatusData(subdomain, org.id);
