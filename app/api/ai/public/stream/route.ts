...
const getConversationKey = (sessionId: string, orgId: string) => `ai:chat:public:${orgId}:${sessionId}`;
...
async function addToConversation(sessionId: string, orgId: string, role: 'user' | 'assistant', content: string) {
  ...
  try {
    ...
    await redis.setex(getConversationKey(sessionId, orgId), CONVERSATION_TTL_SECONDS, JSON.stringify(history.slice(-MAX_CONVERSATION_MESSAGES)));
  } catch {}
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rateLimitResult = await rateLimit(rateLimitKey, { maxRequests: 50, windowSeconds: 60 * 60 });
    ...
    const kbArticles = await db.query.kbArticles.findMany({
      where: (articles, { and, eq, isNotNull }) => and(
