...
const getConversationKey = (sessionId: string, userId: string) => `ai:chat:admin:${userId}:${sessionId}`;
...
async function getConversationHistory(sessionId: string, userId: string): Promise<ConversationMessage[]> {
  if (!redis) return [];
  try {
...
async function addToConversation(sessionId: string, userId: string, role: 'user' | 'assistant', content: string) {
  if (!redis) return;
  try {
    const history = await getConversationHistory(sessionId, userId);
    history.push({ role, content, timestamp: Date.now() });
    await redis.setex(getConversationKey(sessionId, userId), CONVERSATION_TTL_SECONDS, JSON.stringify(history.slice(-MAX_CONVERSATION_MESSAGES)));
  } catch (error) {
  ...
  try {
    ...
    if (guardResult.shouldBlock) {
      ...
      const stream = new ReadableStream({
        start(controller) {
          controller.enqueue(encoder.encode(`data: ${JSON.stringify({ text: getSafeResponse(guardResult.threats), done: false })}\n\n`));
    ...
    const conversationHistory = await getConversationHistory(sessionId, session.user.id);
    const formattedHistory = conversationHistory.length > 0
      ? conversationHistory.map(m => `${m.role.toUpperCase()}: ${m.content}`).join('\n\n')
      : 'No previous conversation.';

    // 7. Build system prompt
    const systemPrompt = ADMIN_SYSTEM_PROMPT.replace('{context_data}', `Session: ${sessionId}\nContext: ${parsed.data.context}`);
    const systemPromptHash = createHash('sha256').update(systemPrompt).digest('hex');

    // 8. Save user query
    await addToConversation(sessionId, session.user.id, 'user', guardResult.sanitizedInput);
        ...
        try {
          for await (const chunk of generator) {
            ...
            if (chunk.done) {
              ...
              await addToConversation(sessionId, session.user.id, 'assistant', fullResponse);
