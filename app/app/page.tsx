...
export default async function DashboardPage() {
  ...
  const [
    ...
    db
      ...
      .from(organizations)
      .where(
        and(eq(organizations.isActive, true), isNull(organizations.deletedAt)),
      ),
    db
      .select({
        status: services.monitoringStatus,
  ...
  const monitoredTotal = monitoredServices.reduce(
  ...
  const unhealthyServices = monitoredServices
    .filter(
      (row) =>
        row.status && !["OPERATIONAL", "OK", "ACTIVE"].includes(row.status),
    )
    .reduce((total, row) => total + Number(row.count || 0), 0);
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard
          label="Active queue"
          value={activeQueue}
          detail="New, open, in progress, waiting"
          href="/app/tickets"
          icon={Ticket}
        />
        <StatCard
          label="Public intake"
          value={publicIntakeCount}
          detail="Requests without an organization"
        ...
        <StatCard
          ...
          detail="Background work requiring review"
          href="/app/admin/jobs"
          icon={AlertTriangle}
        />
      </section>
                ...
                {newestTickets.map((ticket) => (
                  <tr key={ticket.id}>
                    ...
                    <td className="py-3 pr-4 text-slate-600 dark:text-slate-300">
                      {ticket.organization?.customerId ||
            ...
            <CardContent className="space-y-3 text-sm">
              <div className="flex items-center justify-between rounded-md border border-slate-200 p-3 dark:border-slate-800">
                <span className="flex items-center gap-2">
                  <Building2 className="h-4 w-4 text-slate-400" /> Active
                  tenants
                </span>
                <span className="font-semibold">
                  {Number(activeOrgs[0]?.count || 0)}
                </span>
              </div>
              <div className="flex items-center justify-between rounded-md border border-slate-200 p-3 dark:border-slate-800">
                <span className="flex items-center gap-2">
                  <Database className="h-4 w-4 text-slate-400" /> Monitored
                  services
                </span>
              ...
              <div className="flex items-center justify-between rounded-md border border-slate-200 p-3 dark:border-slate-800">
                <span className="flex items-center gap-2">
                  <AlertTriangle className="h-4 w-4 text-slate-400" /> Service
                  warnings
                </span>
