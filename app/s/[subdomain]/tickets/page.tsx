...
export default async function TicketsPage({ params }: TicketsPageProps) {
  ...
  const userTickets = await db.query.tickets.findMany({
    where: userMembership.role === 'CUSTOMER_ADMIN'
  ...
  const formattedTickets = userTickets.filter((ticket) => ticket.status !== 'MERGED').map(ticket => ({
    ...ticket,
    status: ticket.status as 'NEW' | 'OPEN' | 'IN_PROGRESS' | 'WAITING_ON_CUSTOMER' | 'RESOLVED' | 'CLOSED',
    ...
    requester: ticket.requester || (session.user ? { 
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold text-slate-950 tracking-tight">
            {userMembership.role === 'CUSTOMER_ADMIN' ? 'Team Requests' : 'My Requests'}
          </h1>
