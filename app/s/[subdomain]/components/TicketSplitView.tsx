...
export function TicketSplitView({
  subdomain: _subdomain,
  initialTickets,
}: TicketSplitViewProps) {
  ...
  const handleAIAnalysis = async () => {
    ...
    try {
      const res = await fetch(
        `/api/ai/ticket-summary?ticketId=${selectedTicket.id}`,
      );
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!selectedTicketId && filteredTickets.length > 0) {
        ...
        if (e.key === "j" || e.key === "ArrowDown") {
          e.preventDefault();
          const nextIndex =
            currentIndex < filteredTickets.length - 1 ? currentIndex + 1 : 0;
          handleSelectTicket(filteredTickets[nextIndex].id);
        }
        if (e.key === "k" || e.key === "ArrowUp") {
          e.preventDefault();
          const prevIndex =
            currentIndex > 0 ? currentIndex - 1 : filteredTickets.length - 1;
        ...
        {selectedTicket ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {/* Linked Assets Panel */}
              {selectedTicket.linkedAssets &&
                selectedTicket.linkedAssets.length > 0 && (
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected &&
          "bg-orange-50 hover:bg-orange-50 border-l-4 border-orange-500",
      )}
    >
...
function MessageBubble({
  ...
  return (
    <div
      className={cn(
        "flex gap-3",
        type === "requester" ? "flex-row" : "flex-row-reverse",
      )}
    >
      ...
      <div className={cn("max-w-[80%] rounded-2xl px-4 py-3", styles[type])}>
        <div className="flex items-center gap-2 mb-1">
          ...
          {type === "internal" && (
            <Badge
              variant="outline"
              className="text-[10px] h-4 border-yellow-400 text-yellow-700"
            >
              Internal
            </Badge>
          )}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-8">
      ...
      <p className="text-gray-500 max-w-sm">
        Choose a ticket from the list on the left to view the conversation and
        reply
      </p>
