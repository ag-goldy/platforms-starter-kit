...
export default async function CustomerServicesPage({
  ...
  try {
    ...
    const monitoredServices = servicesWithHistory.filter(
      (s) => s.monitoringEnabled,
    );
    const criticalCount = monitoredServices.filter(
      (s) => s.monitoringStatus === "CRITICAL",
    ).length;
    const degradedCount = monitoredServices.filter(
      (s) => s.monitoringStatus === "DEGRADED",
    ).length;
    const operationalCount = monitoredServices.filter(
      (s) => s.monitoringStatus === "OPERATIONAL",
    ).length;
      ...
      <ServicesRefresh refreshInterval={60000}>
        <div className="mx-auto max-w-5xl space-y-6">
          ...
          {monitoredServices.length > 0 && (
            <Card
              className={`rounded-3xl border shadow-sm animate-in fade-in-0 slide-in-from-bottom-2 duration-300 delay-150 ${
                criticalCount > 0
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  <div className="sm:text-right">
                    <div className="text-3xl font-bold text-gray-900 leading-none">
                      {monitoredServices.length > 0
                        ? Math.round(
                            (operationalCount / monitoredServices.length) * 100,
                          )
                        : 100}
                      %
                    </div>
          ...
          <div className="space-y-3 animate-in fade-in-0 slide-in-from-bottom-2 duration-300 delay-200">
            {servicesWithHistory.length === 0 ? (
              ...
              servicesWithHistory.map((svc) => (
                <Card
                  key={svc.id}
                  className="overflow-hidden rounded-3xl border-gray-100 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
                >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          {svc.monitoringEnabled ? (
                            <Badge
                              className={getStatusColor(svc.monitoringStatus)}
                              variant="outline"
                            >
                        ...
                        {/* Monitoring Metrics */}
                        {svc.monitoringEnabled &&
                          svc.monitoringStatus !== "ERROR" && (
                            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 mt-4">
                              {svc.uptimePercentage && (
                                <div className="flex items-center gap-1">
                                  <TrendingUp className="h-4 w-4 text-green-600" />
                                  <span>
                                    {parseFloat(svc.uptimePercentage).toFixed(
                                      2,
                                    )}
                                    % uptime
                                  </span>
                        ...
                        {/* Active Alerts */}
                        {svc.zabbixTriggers &&
                          Array.isArray(svc.zabbixTriggers) &&
                          svc.zabbixTriggers.length > 0 && (
          ...
          <div className="text-xs text-gray-500 pt-4 border-t border-gray-100 animate-in fade-in-0 duration-300 delay-300">
            <p>
              Status indicators show real-time monitoring data from our
