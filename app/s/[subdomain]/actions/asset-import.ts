...
export async function batchImportAssetsAction(
  ...
  const areaMap = new Map(
    orgAreas.map((a) => [
      `${a.site?.name?.toLowerCase() || ""}/${a.name.toLowerCase()}`,
      a.id,
    ]),
  );
  const areaNameMap = new Map(
    orgAreas.map((a) => [a.name.toLowerCase(), { id: a.id, siteId: a.siteId }]),
  );
 ...
 */
export async function generateAssetImportTemplate(
  orgId: string,
): Promise<string> {
  await requireOrgMemberRole(orgId, ["CUSTOMER_ADMIN"]);
 ...
 */
export async function parseAssetImportCSV(
  csvContent: string,
