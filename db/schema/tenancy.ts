import { pgTable, uuid, text, timestamp, jsonb, integer, check, customType } from 'drizzle-orm/pg-core';
...
export const organizations = pgTable('organizations', {
  ...
  return [
    ...
    check('status_enum', sql`${table.status} IN ('active', 'suspended', 'deleted')`),
  ];
});
...
export const orgSettings = pgTable('org_settings', {
  ...
  return [
    check('pii_policy_enum', sql`${table.piiPolicy} IN ('strict', 'standard', 'off')`),
  ];
});
