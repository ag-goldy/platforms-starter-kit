import { pgTable, uuid, text, timestamp, check, jsonb, integer, boolean, smallint } from 'drizzle-orm/pg-core';
...
export const slaPolicies = pgTable('sla_policies', {
  ...
  pauseOnStatus: text('pause_on_status').array().default(['pending', 'on_hold']),
...
export const escalationRules = pgTable('escalation_rules', {
  id: uuid('id').primaryKey().defaultRandom(),
  orgId: uuid('org_id').notNull().references(() => organizations.id, { onDelete: 'cascade' }),
  slaPolicyId: uuid('sla_policy_id').references(() => slaPolicies.id, { onDelete: 'cascade' }),
  ...
  return [
    check('trigger_enum', sql`${table.trigger} IN ('response_warn', 'response_breach', 'resolution_warn', 'resolution_breach', 'no_activity')`),
  ];
});
