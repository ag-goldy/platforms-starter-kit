import { pgTable, uuid, text, timestamp, check, customType, bigint, jsonb, boolean } from 'drizzle-orm/pg-core';
...
export const platformAdmins = pgTable('platform_admins', {
  ...
  return [
    check('role_enum', sql`${table.role} IN ('SUPER_ADMIN', 'ADMIN', 'SUPPORT')`),
  ];
});
...
export const users = pgTable('users', {
  ...
  return [
    check('status_enum', sql`${table.status} IN ('active', 'locked', 'deleted')`),
  ];
});
...
export const sessions = pgTable('sessions', {
  ...
  return [
    check('kind_enum', sql`${table.kind} IN ('tenant', 'platform', 'impersonation')`),
    check('exclusive_user', sql`(${table.userId} IS NOT NULL AND ${table.platformAdminId} IS NULL) OR (${table.userId} IS NULL AND ${table.platformAdminId} IS NOT NULL)`),
  ];
});
...
export const magicLinks = pgTable('magic_links', {
  ...
  return [
    check('purpose_enum', sql`${table.purpose} IN ('login', 'email_verify', 'reset_password', 'invite')`),
  ];
});
