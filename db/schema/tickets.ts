...
export const tickets = pgTable(
  "tickets",
  {
    ...
    responseBreachedAt: timestamp("response_breached_at", {
      withTimezone: true,
    }),
  ...
  (table) => {
    return [
      check(
        "type_check",
        sql`${table.type} IN ('incident','request','problem','change')`,
      ),
      check(
        "status_check",
        sql`${table.status} IN ('new','open','pending','on_hold','resolved','closed','merged')`,
      ),
      check("priority_check", sql`${table.priority} IN ('p1','p2','p3','p4')`),
      check(
        "source_check",
        sql`${table.source} IN ('portal','email','phone','chat','api','automation')`,
      ),
...
export const ticketMessages = pgTable(
  ...
  (table) => {
    return [
      check(
        "author_kind_check",
        sql`${table.authorKind} IN ('user','system','bot')`,
      ),
      check(
        "visibility_check",
        sql`${table.visibility} IN ('public','internal','system')`,
      ),
      check(
        "channel_check",
        sql`${table.channel} IN ('portal','email','api','automation')`,
      ),
    ];
  },
);
...
export const ticketLinks = pgTable(
  ...
  (table) => {
    return [
      ...
      check(
        "link_type_check",
        sql`${table.linkType} IN ('related','duplicate','blocks','blocked_by','parent','child')`,
      ),
    ];
  },
);
...
export const ticketAssets = pgTable(
  ...
  (table) => {
    return [primaryKey({ columns: [table.ticketId, table.assetId] })];
  },
);
...
export const ticketWatchers = pgTable(
  ...
  (table) => {
    return [primaryKey({ columns: [table.ticketId, table.userId] })];
  },
);
