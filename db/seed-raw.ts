...
async function seed() {
  ...
  // Create admin user
  const agPasswordHash = await bcrypt.hash(
    NEW_PASSWORDS["ag@agrnetworks.com"],
    10,
  );
  ...
  try {
    ...
    const existing =
      await sql`SELECT id FROM users WHERE email = 'ag@agrnetworks.com'`;
  ...
  // Create help user
  const helpPasswordHash = await bcrypt.hash(
    NEW_PASSWORDS["help@agrnetworks.com"],
    10,
  );

  try {
    const existing =
      await sql`SELECT id FROM users WHERE email = 'help@agrnetworks.com'`;
