...
export const organizations = pgTable("organizations", {
  ...
  retentionPolicy: text("retention_policy").$type<
    "KEEP_FOREVER" | "DELETE_AFTER_DAYS" | "ANONYMIZE_AFTER_DAYS" | null
  >(),
...
export const services = pgTable(
  "services",
  {
    ...
    zabbixTriggers: jsonb("zabbix_triggers")
      .$type<ZabbixTrigger[]>()
      .default([]),
  ...
  (table) => ({
    uniqueOrgSlug: unique("services_org_slug_unique").on(
      table.orgId,
      table.slug,
    ),
  }),
...
export const requestTypes = pgTable(
  "request_types",
  {
    ...
    requiredAttachments: boolean("required_attachments")
      .default(false)
      .notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
...
export const areas = pgTable(
  ...
  (table) => ({
    uniqueSiteName: unique("areas_site_name_unique").on(
      table.siteId,
      table.name,
    ),
  }),
...
export const assets = pgTable("assets", {
  ...
  accessUrls: jsonb("access_urls").$type<
...
export const attachments = pgTable("attachments", {
  ...
  uploadedByPlatformAdmin: uuid("uploaded_by_platform_admin").references(
    () => platformAdmins.id,
    { onDelete: "set null" },
  ),
...
export const ticketSubtasks = pgTable("ticket_subtasks", {
  ...
  assignedToPlatformAdmin: uuid("assigned_to_platform_admin").references(
    () => platformAdmins.id,
    { onDelete: "set null" },
  ),
  createdBy: uuid("created_by").references(() => users.id, {
  ...
  completedByPlatformAdmin: uuid("completed_by_platform_admin").references(
    () => platformAdmins.id,
    { onDelete: "set null" },
  ),
...
export const ticketDependencies = pgTable(
  ...
  (table) => ({
    ...
    uniqueDependency: unique("unique_dependency").on(
      table.ticketId,
...
export const userInvitations = pgTable("user_invitations", {
  ...
  invitedByPlatformAdmin: uuid("invited_by_platform_admin").references(
    () => platformAdmins.id,
    { onDelete: "set null" },
  ),
...
export const orgAssetStatusesRelations = relations(
  orgAssetStatuses,
  ({ one }) => ({
    organization: one(organizations, {
...
export const zabbixConfigs = pgTable(
  "zabbix_configs",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    orgId: uuid("org_id")
...
export const statuspageConfigs = pgTable(
  "statuspage_configs",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    orgId: uuid("org_id")
    ...
    componentMappings: jsonb("component_mappings")
...
export const statuspageConfigsRelations = relations(
  statuspageConfigs,
  ({ one }) => ({
    organization: one(organizations, {
...
export const serviceMonitoringHistoryRelations = relations(
...
export const internalGroupsRelations = relations(
  internalGroups,
  ({ many, one }) => ({
...
export const ticketTagAssignmentsRelations = relations(
...
export const ticketDependenciesRelations = relations(
  ticketDependencies,
  ({ one }) => ({
    ticket: one(tickets, {
...
export type ServiceMonitoringHistory =
  typeof serviceMonitoringHistory.$inferSelect;
export type NewServiceMonitoringHistory =
  typeof serviceMonitoringHistory.$inferInsert;
...
export type InternalGroupMembership =
  typeof internalGroupMemberships.$inferSelect;
export type NewInternalGroupMembership =
  typeof internalGroupMemberships.$inferInsert;
...
export const ticketWatchers = pgTable(
  ...
  (table) => ({
    uniqueUserWatcher: unique("unique_user_watcher").on(
      table.ticketId,
      table.userId,
    ),
    uniqueAdminWatcher: unique("unique_admin_watcher").on(
      table.ticketId,
...
export const kbArticles = pgTable(
  "kb_articles",
  {
    ...
    submittedById: uuid("submitted_by_id").references(() => users.id, {
      onDelete: "set null",
    }), // Track actual submitter if anonymous
...
export const ticketAssignmentRules = pgTable("ticket_assignment_rules", {
  ...
  internalGroupId: uuid("internal_group_id").references(
    () => internalGroups.id,
    { onDelete: "set null" },
...
export const escalationRules = pgTable("escalation_rules", {
  ...
  // Scope
  applicablePriorities: jsonb("applicable_priorities")
    .$type<("P1" | "P2" | "P3" | "P4")[]>()
    .default(["P1", "P2", "P3", "P4"]),
...
export const maintenanceWindows = pgTable("maintenance_windows", {
  ...
  affectedServiceIds: jsonb("affected_service_ids")
    .$type<string[]>()
    .default([]),
  ...
  createdTicketId: uuid("created_ticket_id").references(() => tickets.id, {
    onDelete: "set null",
  }),
...
export const kbArticleVersionsRelations = relations(
  kbArticleVersions,
  ({ one }) => ({
...
export const kbArticleTemplatesRelations = relations(
  kbArticleTemplates,
  ({ one }) => ({
    org: one(organizations, {
...
export const ticketAssignmentRulesRelations = relations(
...
export const userSessionsExtendedRelations = relations(
  userSessionsExtended,
  ({ one }) => ({
    user: one(users, {
...
export const scheduledTickets = pgTable("scheduled_tickets", {
  ...
  parentScheduleId: uuid("parent_schedule_id").references(
    (): AnyPgColumn => scheduledTickets.id,
    { onDelete: "set null" },
  ),
...
export const scheduledTicketsRelations = relations(
  scheduledTickets,
  ({ one }) => ({
    org: one(organizations, {
...
export const orgAIConfigs = pgTable("org_ai_configs", {
  ...
  allowTicketSummaries: boolean("allow_ticket_summaries")
    .default(false)
    .notNull(),
  ...
  // Content filtering
  blockPIIInResponses: boolean("block_pii_in_responses")
    .default(true)
    .notNull(),
  ...
  includeInternalNotesInAI: boolean("include_internal_notes_in_ai")
    .default(false)
    .notNull(),
...
export const scheduledTicketActionsRelations = relations(
  scheduledTicketActions,
  ({ one }) => ({
    ticket: one(tickets, {
...
export const orgQuotas = pgTable("org_quotas", {
  ...
  maxAIQueriesPerMonth: integer("max_ai_queries_per_month")
    .notNull()
    .default(100),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export type NewScheduledTicketAction =
  typeof scheduledTicketActions.$inferInsert;
