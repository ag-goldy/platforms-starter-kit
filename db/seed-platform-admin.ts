...
async function seed() {
  ...
  for (const admin of PLATFORM_ADMINS) {
    ...
    try {
      ...
      const existing = await sql`SELECT id FROM platform_admins WHERE email = ${admin.email}`;
      ...
      if (existing.length > 0) {
        ...
        `;
        console.log('✓ Updated platform admin:', admin.email, `(${admin.role})`);
      } else {
        await sql`
        ...
        `;
        console.log('✓ Created platform admin:', admin.email, `(${admin.role})`);
      }
    } catch (e) {
