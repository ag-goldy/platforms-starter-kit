 ...
 */
async function resolveOrgBySlug(slug: string): Promise<{ id: string; subdomain: string; isActive: boolean; deletedAt: Date | null } | null> {
  try {
    ...
    return result[0] as { id: string; subdomain: string; isActive: boolean; deletedAt: Date | null };
  } catch {
    return null;
  }
}
...
export async function middleware(request: NextRequest) {
  ...
  const rootRoutes = ['/', '/login', '/signup', '/forgot-password', '/reset-password', '/magic', '/passkey/register'];
  const platformPrefixes = ['/admin', '/api'];
  const appPrefixes = ['/app', '/s'];
  
  const isRootRoute = rootRoutes.includes(pathname);
  const isPlatformRoute = platformPrefixes.some(prefix => pathname.startsWith(prefix));

  if (isRootRoute || isPlatformRoute || appPrefixes.some(prefix => pathname === prefix || pathname.startsWith(`${prefix}/`))) {
  ...
  if (pathParts.length > 0) {
    ...
    if (pathParts[1] === 'portal') {
      ...
      const redirectPath = rest ? `/s/${org.subdomain}/${rest}` : `/s/${org.subdomain}`;
